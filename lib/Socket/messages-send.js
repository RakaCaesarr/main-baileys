...
const makeMessagesSocket = (config) => {
    ...
    const rakaInstance = new raka(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = rakaInstance.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await rakaInstance.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await rakaInstance.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await rakaInstance.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await rakaInstance.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await rakaInstance.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await rakaInstance.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await rakaInstance.handleGroupStory(content, jid, quoted)
                    case 'STICKER_PACK':
                        const packContent = await rakaInstance.handleStickerPack(content, jid, quoted);
